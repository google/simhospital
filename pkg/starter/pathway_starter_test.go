...
func TestServeHTTP_AllProdPathwaysCanBeStartedByName(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("ParseOrderProfiles(%s, %+v) failed with %v", test.PublicOrderProfilesConfig, hl7Config, err)
	}
...
func newTestPathwayStarter(t *testing.T, pathways map[string]pathway.Pathway, cfg hospital.Config) *pathwayStarter {
	...
	h := testhospital.WithTime(t, testhospital.Config{Config: cfg, ConfigFiles: test.HospitalFiles[test.Test]}, now)
	p := &pathway.Parser{
