...
func (g *Generator) Random() *message.CodedElement {
	e := g.DiscreteDistribution.Random()
	if e == nil {
		return nil
	}
	return e.(*message.CodedElement)
}
...
func (g *Generator) nullTimeOrRandom(dt *pathway.DateTime) message.NullTime {
	if dt == nil {
		return g.dateGenerator.Random(g.clock.Now())
	}
	if t := dt.GetDateTime(g.clock.Now()); t != nil {
		return message.NewValidTime(*t)
	}
	return message.NewInvalidTime()
}
...
type DateGenerator interface {
	...
	Random(now time.Time) message.NullTime
}
...
func (s SimpleDateGenerator) Random(now time.Time) message.NullTime {
	days := rand.Int63n(364) + 1
	timeFromNow := -time.Duration(days) * 24 * time.Hour
	return message.NewValidTime(now.Add(timeFromNow))
}
